...
const SearchResults = () => {
  ...
  const sortedVehicles = [...vehicles].sort((a, b) => {
    switch (sortBy) {
      case "price_asc":
        return (a.pricePerDay || a.salePrice || 0) - (b.pricePerDay || b.salePrice || 0);
      case "price_desc":
        return (b.pricePerDay || b.salePrice || 0) - (a.pricePerDay || a.salePrice || 0);
      case "rating_desc":
  ...
  if (loading) {
    return <div className="container mx-auto px-4 py-8">Searching listings...</div>;
  }

  if (error) {
    return <div className="container mx-auto px-4 py-8 text-red-600">Error loading search results: {error}</div>;
  }
        ...
        <main className="lg:w-3/4">
          <div className="mb-6">
            <h1 className="text-2xl font-bold">
              {sortedVehicles.length} results for "{query}"
            </h1>
          </div>

          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-gray-600">{sortedVehicles.length} vehicles found</div>
