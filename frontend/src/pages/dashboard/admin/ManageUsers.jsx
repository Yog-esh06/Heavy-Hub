// frontend/src/pages/dashboard/admin/ManageListings.jsx
...
const ManageListings = () => {
    ...
    const fetchListings = async () => {
      try {
        ...
        if (activeTab === 'pending') {
          q = query(collection(db, 'vehicles'), where('isVerified', '==', false));
        } else if (activeTab === 'reported') {
          q = query(collection(db, 'vehicles'), where('reportCount', '>', 0));
        }
  ...
  const handleVerifyToggle = async (listingId, currentStatus) => {
    try {
      await updateDoc(doc(db, 'vehicles', listingId), { isVerified: !currentStatus });
      setListings(prev => prev.map(l => l.id === listingId ? { ...l, isVerified: !currentStatus } : l));
